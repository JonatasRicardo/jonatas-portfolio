  ...
  return (
    <section className="bg-white px-6 py-16 lg:py-24">
      <div className="mx-auto max-w-2xl">
        <p className="text-xs font-bold tracking-widest text-[#E8820C] uppercase">Como funciona</p>
        <h2 className="mt-3 font-serif text-2xl leading-snug font-bold text-[#1a1a1a] lg:text-3xl">
  ...
  return (
    <div className="border-b border-[#1a1a1a]/10">
      ...
      >
        ...
        <span className={cn("shrink-0 text-[#E8820C] transition-transform duration-200", open && "rotate-45")}>
          <svg fill="none" height="20" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" width="20">
            <path d="M12 5v14M5 12h14" strokeLinecap="round" />
          </svg>
        </span>
      </button>
      {open && <p className="pb-5 text-sm leading-relaxed text-[#1a1a1a]/65 lg:text-base">{answer}</p>}
    </div>
  )
}
...
export function ConsultoriaWebCtaFinal() {
  const whatsappUrl = useMemo(() => `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(defaultMessage)}`, [])

  return (
    <section className="bg-[#F5EDD8] px-6 pt-0 pb-20 lg:pb-28">
      <div className="mx-auto max-w-2xl">
